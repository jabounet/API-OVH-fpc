  ...
  begin
    ...
    begin
      ...
      case current_state of
        end_ringing, end_calling: if endTime = startTime then
            endTime := incsecond(now, 10);
        start_ringing: if endTime = startTime then
            endTime := incsecond(now, 60);
        start_calling: if endTime = startTime then
            endTime := incsecond(now, 7200);
      end;
      if now >= endTime then
        keep := False;
    end;
        ...
        begin
          ...
          begin
            ...
            if (current_state <> idle) and (current_state <> registered) then
              log(DateTimetoStr(StartTime) + '@' + CallId + ':' +
                calling + '>' + called + ':' + Statetostr(current_state));
          end;
